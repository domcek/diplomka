...
#	raw_input('Press <ENTER> to continue')
	...
	for i in range(n):
		...
		error_down.append(abs(m(12+i*12).error[0]-m(12+i*12).values[0])) #xspec returns errors in format with down and up limit, it is necessary to substract average fitted value to get errors into python
		error_up.append(abs(m(12+i*12).error[1]-m(12+i*12).values[0]))


	raw_input('Press <ENTER> to continue')

	
	return x,val,error_down,error_up
...
def not_broken(n,k,spin,theta,ms,start,fitfinish,modelfinish,lineE,alpha,beta,Rbr,z,limb,norm,phoindex,phonorm,nh,err_fit,broken,model):
	...
	if model == -1:	
		x_in,y_in,error_down_in,error_up_in = fit_kyrpow(n,r_in,S_in,spin,theta,ms,start,fitfinish,lineE,alpha,beta,Rbr,z,limb,norm,phoindex,phonorm,nh,err_fit)

	if model == 0:
		x_in,y_in,error_down_in,error_up_in = fit_kyrpow_S(n,r_in,S_in,spin,theta,ms,start,fitfinish,lineE,alpha,beta,Rbr,z,limb,norm,phoindex,phonorm,nh,err_fit)
	...
	errl_up_in = []
	if err_fit == True:
		for i in range(len(error_down_in)):
			errl_down_in.append(abs(derivation_const*error_down_in[i]/y_in[i])) #relative error of loglog scale
			errl_up_in.append(abs(derivation_const*error_up_in[i]/y_in[i])) #relative error of loglog scale



	popt_in, pcov_in = curve_fit(powerlaw, xl_in, yl_in)
	print popt_in
	return x_in,y_in,error_down_in,error_up_in,xl_in,yl_in,errl_down_in,errl_up_in,popt_in,pcov_in
...
#	AllModels.initpackage("ky", "lmodel.dat" ,"/mnt/31660B856FD2FABA/xspec/models/KY_code_-1/ -udmget64")
	...
	if broken == False: 
		x,y,error_down,error_up,xl,yl,errl_down,errl_up,popt,pcov = not_broken(n,k,spin,theta,ms,start,fitfinish,modelfinish,lineE,alpha,beta,Rbr,z,limb,norm,phoindex,phonorm,nh,err_fit,broken,model)
...
#	AllModels.initpackage("ky", "lmodel.dat" ,"/mnt/31660B856FD2FABA/xspec/models/KY_code_0/ -udmget64")
	...
	#ylim([10**(-22),10**4])
	if err_fit == True: errorbar(x,y, yerr = [error_down,error_up], marker = "+", linestyle = "None") #weird stuff
...
nh = 1
err_fit = True
broken = False #if false, need to have modelfinish and fitfinish same
